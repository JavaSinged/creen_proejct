...
export default function StoreView() {
  ...
  // ✅ 메뉴 객체 전체를 넘겨서 모달이 axios 없이 바로 쓸 수 있게
        ...
        <div className={styles.menu_grid}>
          {filteredMenu.map((menu) => (
            <div
              ...
              onClick={() => handleMenuClick(menu)} // ✅ 객체 전달
            >
              <div className={styles.menu_image}></div>
              <div className={styles.menu_info}>
                <span className={styles.menu_title}>{menu.name}</span>
          ...
          {filteredMenu.length === 0 && <p>해당 메뉴가 없습니다.</p>}
        </div>
      </div>

      {/* ✅ menuId 대신 menuData 객체를 바로 전달 */}
      <MenuModal
        isOpen={isModalOpen}
